...
public class AuthController : ControllerBase
{
    ...
    [HttpPut("/users/{id}")]
    [Authorize]
    public IActionResult UpdateProfile([FromBody] UpdateProfileRequest request, [FromRoute] string? id = null)
    {
        EnsureConfiguredUsersSeeded();
        ...
        if (!string.IsNullOrWhiteSpace(id) && !string.Equals(id, user.Id, StringComparison.OrdinalIgnoreCase))
        {
            return Forbid();
        }
        ...
        if (!string.IsNullOrWhiteSpace(request.Name))
        {
            ...
            user.Handle = request.Name.Trim().ToLowerInvariant().Replace(" ", string.Empty);
        }

        user.Bio = request.Bio?.Trim() ?? string.Empty;
        user.Location = request.Location?.Trim() ?? string.Empty;
        user.Website = request.Website?.Trim() ?? string.Empty;
        ...
        {
            // Keep profile flow functional even when local file persistence is unavailable.
        }
