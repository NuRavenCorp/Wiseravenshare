// Application/Services/Assistant/WebGroundingService.cs
...
{
    public string Provider { get; set; } = "brave"; // brave | bing | serpapi
...
public class WebGroundingService : IWebGroundingService
{
    ...
    public async Task<WebSnippet[]> SearchAsync(string query, int topN, CancellationToken ct = default)
    {
        try
        {
            ...
            if (_opts.ExtractPageContent)
            {
                foreach (var s in snippets.ToList())
                {
                    try
                    {
                        var html = await _http.GetStringAsync(s.Url, ct);
                        var content = ExtractMainText(html);
                        if (content.Length > _opts.ExtractMaxChars)
                            content = content[.._opts.ExtractMaxChars];
                        snippets[snippets.IndexOf(s)] = s with { Snippet = content };
                    }
                    catch { /* ignore */ }
                }
            }
    ...
    private static string ExtractMainText(string html)
    {
        ...
        doc.LoadHtml(html);
        foreach (var n in doc.DocumentNode.SelectNodes("//script|//style|//nav|//header|//footer|//aside")
                     ?? Enumerable.Empty<HtmlNode>())
            n.Remove();
        var main = doc.DocumentNode.SelectSingleNode("//main") ?? doc.DocumentNode.SelectSingleNode("//article") ?? doc.DocumentNode;
        var text = System.Net.WebUtility.HtmlDecode(main.InnerText);
