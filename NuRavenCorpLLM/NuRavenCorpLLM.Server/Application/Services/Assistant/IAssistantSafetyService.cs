// Application/Services/Assistant/AssistantSafetyService.cs
...
public class AssistantSafetyService : IAssistantSafetyService
{
    ...
    public Task<bool> IsSafeAsync(string userText, CancellationToken ct = default)
    {
        ...
        if (Options.IsMatch(userText)) return Task.FromResult(false);
    ...
    public Task<bool> IsSafeForTrainingAsync(string a, string? b = null)
    {
        ...
        if (Options.IsMatch(a)) return Task.FromResult(false);
        if (!string.IsNullOrEmpty(b) && Options.IsMatch(b)) return Task.FromResult(false);
        return Task.FromResult(true);
    }
}