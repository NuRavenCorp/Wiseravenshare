// Application/Services/Assistant/AssistantOrchestrator.cs
...
namespace NuRavenCorpLLM.Application.Services;
...
public record AssistantStreamEvent(string Type, string? Delta, AssistantMessage? Message, string? Error);
...
- Use the WiseRavenShare platform context and connected local folder knowledge when relevant.
";
...
public class AssistantOrchestrator : IAssistantOrchestrator
{
        ...
        bool isVoice = false, CancellationToken ct = default)
    {
        ...
        // 7. Learning sample (fire and forget)
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        if (!await _safety.IsSafeAsync(userText, ct))
        {
            yield return new AssistantStreamEvent("blocked", null, null, "Content blocked by safety policy");
            yield break;
        }
        ...
        await foreach (var chunk in _openAi.ChatStreamAsync(request, ct))
        {
            if (chunk.Done)
            {
...
{
    Task AddAsync(AssistantConversation conv);
    Task<AssistantConversation?> GetAsync(Guid conversationId);
    Task<AssistantConversation?> GetWithMessagesAsync(Guid conversationId);
    Task UpdateAsync(object conv);
}

public interface IAssistantMessageRepository
{
    Task AddAsync(AssistantMessage msg);
}

public interface IAssistantFeedbackRepository
{
    Task AddAsync(AssistantFeedback msg);
