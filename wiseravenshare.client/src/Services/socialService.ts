...
async function getProviderStatuses(): Promise<SocialProviderStatus[]> {
  const response = await fetch(`${apiBase}/api/social/providers/status`, { method: 'GET' });
  if (!response.ok) {
    throw new Error(await parseError(response, `Failed to load provider status (${response.status})`));
  }
...
export function buildMediaSharePayload(options: {
  ...
  return {
    ...
    publishToInstagram: Boolean(options.publishToInstagram) && (isVideo || isPhoto),
