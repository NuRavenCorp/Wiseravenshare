...
class AuthService {
    ...
    async postAuthWithFallback(path, payload = {}, options = {}) {
        ...
        // Try /api/auth first for reverse-proxy deployments, then /auth for compatibility.
    ...
    async requestProfileWithFallback(method, path, payload = null) {
        const attempts = this.buildProfileAttempts(path);
        const token = this.getToken();
        let lastError = null;
        ...
        const err = new Error('Profile service is currently unavailable. Please try again.');
        err.status = lastError?.status || 0;
        throw err;
    }
    ...
    async getProfile() {
        try {
            const response = await this.requestProfileWithFallback('GET', '/profile');
            if (response) {
    ...
    async updateProfile(userId, updates) {
        try {
            let profile = null;
            try {
                profile = await this.requestProfileWithFallback('PUT', '/profile', updates);
            } catch (error) {
                // Compatibility fallback for older backend routes.
                if (error?.status === 404 && userId) {
                    const legacy = await api.put(`/users/${userId}`, updates);
            ...
            const canonical = await this.getProfile();
            const next = canonical || profile || { ...this.getUser(), ...updates };
            this.setUser(next);
            return next;
        } catch (error) {
            throw this.handleError(error);
