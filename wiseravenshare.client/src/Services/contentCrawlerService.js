 ...
 */
export async function getTrendingContent(options = {}) {
 ...
 */
export async function getEmergingTopics(countryCode = null) {
  const token = getAuthToken();
  if (!token) return [];
 ...
 */
export async function ingestContent(contentData) {
  try {
 ...
 */
export async function updateEngagement(contentId, metrics) {
  try {
 ...
 */
export async function getTrendingByType(contentType, topN = 12) {
  return getTrendingContent({ contentType, topN });
}
...
export function formatContent(content) {
  return {
    ...
    viral: {
      ...
      badge: getViralBadge(content.viralCoefficient),
    },
    tags: content.tags || [],
 ...
 */
export function getViralBadge(coefficient) {
